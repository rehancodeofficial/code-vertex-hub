...
export function AssetsPage() {
  ...
  const filtered = assets.filter((a) => {
    ...
    if (q && !`${a.name} ${a.tag} ${a.serial}`.toLowerCase().includes(q.toLowerCase())) return false;
    return true;
  });
  ...
  return (
    <>
      <PageHeader
        ...
        actions={<Button className="gradient-primary text-primary-foreground shadow-glow"><Plus className="size-4" /> Add asset</Button>}
      />
      ...
      <Card className="glass shadow-elevated p-4">
        <div className="flex flex-col md:flex-row gap-3 mb-4">
          <div className="relative flex-1">
            ...
            <Input placeholder="Search by name, tag, serial…" value={q} onChange={(e) => setQ(e.target.value)} className="pl-9 bg-muted/40" />
          </div>
                ...
                <TableRow key={a.id}>
                  <TableCell>
                    <div className="flex items-center gap-2.5">
                      <div className="size-8 rounded-md bg-muted flex items-center justify-center"><Boxes className="size-4 text-primary" /></div>
                      <div>
                  ...
                  <TableCell className="text-sm font-mono text-muted-foreground">{a.serial}</TableCell>
                  <TableCell className="text-sm">{a.assignedTo ? employeeById(a.assignedTo)?.fullName : <span className="text-muted-foreground">—</span>}</TableCell>
